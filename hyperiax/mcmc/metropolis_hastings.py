...
def metropolis_hastings_(log_target,proposal, data, init, num_samples, burn_in=0, thin=1, rng_key=None, savef=lambda x: x):
    """ MCMC using Metropolis-Hastings algorithm
    ...
    rng_key, subkey = jax.random.split(rng_key)
    current_state = init #proposal(data, init, subkey)
    log_likelihood_current, path = log_target(data,current_state)
    log_likelihoods = []
    samples = []
    paths = []
    accepted_count = 0  # Initialize counter for accepted proposals
    ...
    for i in tqdm(range(num_samples + burn_in)):
        rng_key, subkey = jax.random.split(rng_key)
        # Propose a new state, new noise, new parameters
        proposed_state = proposal(data, current_state, subkey)

        # Compute acceptance probability
        log_likelihood_proposed, cur_path = log_target(data,proposed_state)
        alpha = jnp.minimum(1.0, jnp.exp(log_likelihood_proposed - log_likelihood_current))
    ...
    return log_likelihoods,samples, paths